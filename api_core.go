...
type ApiKeepaliveSourcesRequest struct {
	ctx context.Context
	ApiService *CoreApiService
}

func (r ApiKeepaliveSourcesRequest) Execute() (*http.Response, error) {
	return r.ApiService.KeepaliveSourcesExecute(r)
}

/*
KeepaliveSources Sources - Keepalive
...
Updates sources to keep them running in the background. This can be used to add explicit action, when the built-in keepalives are not sufficient.
 ...
 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiKeepaliveSourcesRequest
*/
func (a *CoreApiService) KeepaliveSources(ctx context.Context) ApiKeepaliveSourcesRequest {
	return ApiKeepaliveSourcesRequest{
		ApiService: a,
		ctx: ctx,
	}
...
// Execute executes the request
func (a *CoreApiService) KeepaliveSourcesExecute(r ApiKeepaliveSourcesRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "CoreApiService.KeepaliveSources")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/core/sources/keepalive"
